...
*/
    fun init_module(admin: &signer) {
        ...
        let resource_account = Signer::address_of(&admin);

        // Create the account NFT collection
        let account_collection = NFT::create_empty_collection(
            &admin,
            "account collection".to_string(),
            "account collection description".to_string(),
            "account collection uri".to_string(),
            false,
            true,
        );
    ...
    let state = State::new();
    move_resource::MoveResource::publish(&state, &resource_account);

    // Create and move the ModuleEventStore resource to the resource account
    let event_store = ModuleEventStore::new();
    move_resource::MoveResource::publish(&event_store, &resource_account);

    // Create and move the GlobalTimeline resource to the resource account
    let global_timeline = GlobalTimeline::new();
    move_resource::MoveResource::publish(&global_timeline, &resource_account);
    ...
    let account_address = AccountMetaData::get_account_address(&username);
    assert(account_address == None, 1);

    // Check if the username, name, and profile pic uri are valid length
    assert(username.len() > 0 && username.len() <= 32, 2);
    assert(name.len() > 0 && name.len() <= 64, 3);
    assert(profile_pic.len() > 0 && profile_pic.len() <= 256, 4);

    // Check if any of the usernames to follow are not registered
    for username_to_follow in usernames_to_follow {
        let account_address = AccountMetaData::get_account_address(&username_to_follow);
        assert(account_address != None, 5);
    }

    // Check if there is a repeated username in the list
    let mut usernames_to_follow_set = std::collections::HashSet::new();
    for username_to_follow in usernames_to_follow.iter() {
        assert(usernames_to_follow_set.insert(username_to_follow), 6);
    }

    // Check if the list contains the username of the account being created
    assert(!usernames_to_follow.contains(&username), 7);

    // Create the account NFT
    let account_collection = NFT::get_collection("account collection").unwrap();
    let account_nft = account_collection.mint(&owner);

    // Create the account metadata
    let account_metadata = AccountMetaData::new(
        username.clone(),
        name.clone(),
        profile_pic.clone(),
        account_nft,
        usernames_to_follow.clone(),
    );

    // Add the account metadata to the state
    State::add_account_metadata(&account_metadata);

    // Emit the AccountCreated event
    let event_data = AccountCreatedEvent::new(account_metadata.clone());
    ModuleEventStore::emit_event(&event_data);

    // Follow the usernames in the list
    for username_to_follow in usernames_to_follow {
        assert(username_to_follow != username, 8);
        AccountMetaData::add_follower(&username_to_follow, &account_metadata);
    }
}
    ...
    */
    ...
    let account_metadata = AccountMetaData::get_account_metadata(&username);
    assert(account_metadata != None, 1);
    let account_metadata = account_metadata.unwrap();

    // Check if the owner account owns the account associated with the username
    assert(account_metadata.owner_address == *owner, 2);

    // Check if the name is valid length
    assert(name.len() > 0 && name.len() <= 64, 3);

    // Update the account metadata
    let updated_account_metadata = account_metadata.update_name(name.clone());

    // Update the account metadata in the state
    State::update_account_metadata(&updated_account_metadata);

    // Emit the AccountUpdated event
    let event_data = AccountUpdatedEvent::new(updated_account_metadata.clone());
    ModuleEventStore::emit_event(&event_data);
}
    ...
    */
    ...
    // Check if the bio is too long
    assert(bio.len() <= 256, 3);

    // Update the account metadata
    let updated_account_metadata = account_metadata.update_bio(bio.clone());
    ...
    */
    ...
    // Check if the profile picture uri is valid length
    assert(profile_picture_uri.len() > 0 && profile_picture_uri.len() <= 256, 3);

    // Update the account metadata
    let updated_account_metadata = account_metadata.update_profile_picture(profile_picture_uri.clone());
    ...
    */
    ...
    // Get the account metadata for the follower account
    let follower_account_metadata = AccountMetaData::get_account_metadata(&follower_username);
    assert(follower_account_metadata != None, 1);
    let follower_account_metadata = follower_account_metadata.unwrap();

    // Check if the owner account owns the follower account
    assert(follower_account_metadata.owner_address == *follower, 2);

    // Get the account metadata for the account to follow
    let account_to_follow_metadata = AccountMetaData::get_account_metadata(&username_to_follow);
    assert(account_to_follow_metadata != None, 3);
    let account_to_follow_metadata = account_to_follow_metadata.unwrap();

    // Check if the username to follow is not the same as the follower username
    assert(username_to_follow != follower_username, 4);

    // Check if the follower account is not already following the account to follow
    assert(!follower_account_metadata.following.contains(&username_to_follow), 5);

    // Update the follower account metadata
    let updated_follower_account_metadata = follower_account_metadata.add_following(username_to_follow.clone());

    // Update the follower account metadata in the state
    State::update_account_metadata(&updated_follower_account_metadata);

    // Emit the AccountFollowed event
    let event_data = AccountFollowedEvent::new(follower_username.clone(), username_to_follow.clone());
    ModuleEventStore::emit_event(&event_data);
}
...
entry fun unfollow(
    ...
    // Get the account metadata for the unfollower account
    let unfollower_account_metadata = AccountMetaData::get_account_metadata(&unfollower_username);
    assert(unfollower_account_metadata != None, 1);
    let unfollower_account_metadata = unfollower_account_metadata.unwrap();

    // Check if the owner account owns the unfollower account
    assert(unfollower_account_metadata.owner_address == *unfollower, 2);

    // Get the account metadata for the account to unfollow
    let account_to_unfollow_metadata = AccountMetaData::get_account_metadata(&username_to_unfollow);
    assert(account_to_unfollow_metadata != None, 3);
    let account_to_unfollow_metadata = account_to_unfollow_metadata.unwrap();

    // Check if the username to unfollow is not the same as the unfollower username
    assert(username_to_unfollow != unfollower_username, 4);

    // Check if the unfollower account is already following the account to unfollow
    assert(unfollower_account_metadata.following.contains(&username_to_unfollow), 5);

    // Update the unfollower account metadata
    let updated_unfollower_account_metadata = unfollower_account_metadata.remove_following(username_to_unfollow.clone());

    // Update the unfollower account metadata in the state
    State::update_account_metadata(&updated_unfollower_account_metadata);

    // Emit the AccountUnfollowed event
    let event_data = AccountUnfollowedEvent::new(unfollower_username.clone(), username_to_unfollow.clone());
    ModuleEventStore::emit_event(&event_data);
}
...
entry fun post(
    ...
    // Check if the content is valid length
    assert(content.len() > 0 && content.len() <= 1024, 3);

    // Create a new post
    let post = Post::new(username.clone(), content.clone());

    // Add the post to the account metadata
    let updated_account_metadata = account_metadata.add_post(post.clone());

    // Update the account metadata in the state
    State::update_account_metadata(&updated_account_metadata);

    // Add the post to the publications
    Publications::add_post(post.clone());

    // Add the post to the global timeline
    GlobalTimeline::add_post(post.clone());

    // Emit the PostPublished event
    let event_data = PostPublishedEvent::new(post.clone());
    ModuleEventStore::emit_event(&event_data);
}
...
entry fun comment(
    ...
    // Get the account metadata for the commenter account
    let commenter_account_metadata = AccountMetaData::get_account_metadata(&commenter_username);
    assert(commenter_account_metadata != None, 1);
    let commenter_account_metadata = commenter_account_metadata.unwrap();

    // Check if the owner account owns the commenter account
    assert(commenter_account_metadata.owner_address == *commenter, 2);

    // Check if the content is valid length
    assert(content.len() > 0 && content.len() <= 1024, 3);

    // Get the account metadata for the post author account
    let post_author_account_metadata = AccountMetaData::get_account_metadata(&post_author_username);
    assert(post_author_account_metadata != None, 4);
    let post_author_account_metadata = post_author_account_metadata.unwrap();

    // Check if the post author account is registered
    assert(post_author_account_metadata.is_registered(), 5);

    // Get the post from the publications
    let post = Publications::get_post(post_id);
    assert(post != None, 6);
    let post = post.unwrap();

    // Check if the post author username matches the post author in the post
    assert(post.author_username == post_author_username, 7);

    // Create a new comment
    let comment = Comment::new(commenter_username.clone(), content.clone());

    // Add the comment to the post
    let updated_post = post.add_comment(comment.clone());

    // Update the post in the publications
    Publications::update_post(post_id, updated_post.clone());

    // Emit the CommentPublished event
    let event_data = CommentPublishedEvent::new(comment.clone(), post_id, post_author_username.clone());
    ModuleEventStore::emit_event(&event_data);
}
...
entry fun like(
    ...
    // Get the account metadata for the liker account
    let liker_account_metadata = AccountMetaData::get_account_metadata(&liker_username);
    assert(liker_account_metadata != None, 1);
    let liker_account_metadata = liker_account_metadata.unwrap();

    // Check if the owner account owns the liker account
    assert(liker_account_metadata.owner_address == *liker, 2);

    // Check if the publication type is valid
    assert(PublicationType::is_valid(publication_type.clone()), 3);

    // Get the account metadata for the publication author account
    let publication_author_account_metadata = AccountMetaData::get_account_metadata(&publication_author_username);
    assert(publication_author_account_metadata != None, 4);
    let publication_author_account_metadata = publication_author_account_metadata.unwrap();

    // Check if the publication author account is registered
    assert(publication_author_account_metadata.is_registered(), 5);

    // Get the publication from the publications
    let publication = Publications::get_publication(publication_type.clone(), publication_id);
    assert(publication != None, 6);
    let publication = publication.unwrap();

    // Check if the liker has already liked the publication
    assert(!publication.has_liked(&liker_username), 7);

    // Create a new like
    let like = Like::new(liker_username.clone());

    // Add the like to the publication
    let updated_publication = publication.add_like(like.clone());

    // Update the publication in the publications
    Publications::update_publication(publication_type.clone(), publication_id, updated_publication.clone());

    // Emit the PublicationLiked event
    let event_data = PublicationLikedEvent::new(like.clone(), publication_type.clone(), publication_id, publication_author_username.clone());
    ModuleEventStore::emit_event(&event_data);
}
...
entry fun unlike(
    ...
    // Get the account metadata for the unliker account
    let unliker_account_metadata = AccountMetaData::get_account_metadata(&unliker_username);
    assert(unliker_account_metadata != None, 1);
    let unliker_account_metadata = unliker_account_metadata.unwrap();

    // Check if the owner account owns the unliker account
    assert(unliker_account_metadata.owner_address == *unliker, 2);
    ...
    // Check if the unliker has already liked the publication
    assert(publication.has_liked(&unliker_username), 7);

    // Remove the like from the publication
    let updated_publication = publication.remove_like(&unliker_username);

    // Update the publication in the publications
    Publications::update_publication(publication_type.clone(), publication_id, updated_publication.clone());

    // Emit the PublicationUnliked event
    let event_data = PublicationUnlikedEvent::new(unliker_username.clone(), publication_type.clone(), publication_id, publication_author_username.clone());
    ModuleEventStore::emit_event(&event_data);
}
...
public fun get_global_timeline(
    ...
    let global_timeline = GlobalTimeline::get_global_timeline();

    // Get the start and end indices for the page
    let start_index = page_size * page_index;
    let end_index = min(start_index + page_size, global_timeline.len());

    // Get the posts for the page
    let posts = global_timeline.slice(start_index, end_index);

    // Get the account metadata for the post authors
    let account_metadata = AccountMetaData::get_account_metadata_batch(posts.map<Post, String>(|post| post.author_username.clone()));

    // Get the likes for the posts
    let likes = posts.map<Post, vector<Like>>(|post| post.likes.clone());

    // Get the viewer's likes for the posts
    let viewer_likes = likes.map<vector<Like>, bool>(|post_likes| post_likes.has_liked(&viewer_username));

    return (account_metadata, posts, viewer_likes);
}
...
public fun get_following_timeline(
    ...
    // Get the following list for the username
    let following_list = Following::get_following_list(&username);

    // Get the start and end indices for the page
    let start_index = page_size * page_index;
    let end_index = min(start_index + page_size, following_list.len());

    // Get the accounts followed by the username for the page
    let followed_accounts = following_list.slice(start_index, end_index);

    // Get the posts for the followed accounts
    let posts = followed_accounts.flat_map<AccountMetaData, vector<Post>>(|account_metadata| {
        let account_posts = Publications::get_account_posts(account_metadata.username.clone());
        account_posts
    });
...
public fun get_account_posts(
    ...
    // Get the account metadata for the username
    let account_metadata = AccountMetaData::get_account_metadata(&username);

    // Get the posts for the account
    let posts = Publications::get_account_posts(username.clone());

    // Get the start and end indices for the page
    let start_index = page_size * page_index;
    let end_index = min(start_index + page_size, posts.len());

    // Get the posts for the page
    let page_posts = posts.slice(start_index, end_index);

    // Get the likes for the posts
    let likes = page_posts.map<Post, vector<Like>>(|post| post.likes.clone());

    // Get the viewer's likes for the posts
    let viewer_likes = likes.map<vector<Like>, bool>(|post_likes| post_likes.has_liked(&viewer_username));

    return (account_metadata, page_posts, viewer_likes);
}
...
public fun get_account_comments(
    ...
    // Get the comments for the account
    let comments = Publications::get_account_comments(username.clone());

    // Get the start and end indices for the page
    let start_index = page_size * page_index;
    let end_index = min(start_index + page_size, comments.len());

    // Get the comments for the page
    let page_comments = comments.slice(start_index, end_index);

    // Get the posts that the comments are on
    let posts = page_comments.map<Comment, Post>(|comment| {
        let post_id = comment.post_id.clone();
        Publications::get_post(post_id)
    });

    // Get the account metadata for the post authors
    let account_metadata_for_posts = AccountMetaData::get_account_metadata_batch(posts.map<Post, String>(|post| post.author_username.clone()));

    // Get the likes for the comments
    let comment_likes = page_comments.map<Comment, vector<Like>>(|comment| comment.likes.clone());

    // Get the viewer's likes for the comments
    let viewer_comment_likes = comment_likes.map<vector<Like>, bool>(|comment_likes| comment_likes.has_liked(&viewer_username));

    // Get the likes for the posts
    let post_likes = posts.map<Post, vector<Like>>(|post| post.likes.clone());

    // Get the viewer's likes for the posts
    let viewer_post_likes = post_likes.map<vector<Like>, bool>(|post_likes| post_likes.has_liked(&viewer_username));

    return (account_metadata, page_comments, posts, account_metadata_for_posts, viewer_comment_likes, viewer_post_likes);
}
...
public fun get_account_liked_posts(
    ...
    // Get the likes for the account
    let likes = Publications::get_account_likes(username.clone());

    // Get the start and end indices for the page
    let start_index = page_size * page_index;
    let end_index = min(start_index + page_size, likes.len());

    // Get the likes for the page
    let page_likes = likes.slice(start_index, end_index);

    // Get the posts for the likes
    let posts = page_likes.map<Like, Post>(|like| {
        let post_id = like.post_id.clone();
        Publications::get_post(post_id)
    });
    ...
    // Get the viewer's likes for the posts
    let viewer_likes = page_likes.map<Like, bool>(|like| like.has_liked(&viewer_username));

    return (account_metadata, posts, account_metadata_for_posts, viewer_likes);
}
...
public fun get_account_liked_comments(
    ...
    // Get the comments for the likes
    let comments = page_likes.map<Like, Comment>(|like| {
        let comment_id = like.comment_id.clone();
        Publications::get_comment(comment_id)
    });

    // Get the posts that the comments are on
    let posts = comments.map<Comment, Post>(|comment| {
        let post_id = comment.post_id.clone();
        Publications::get_post(post_id)
    });

    // Get the account metadata for the comment authors
    let account_metadata_for_comments = AccountMetaData::get_account_metadata_batch(comments.map<Comment, String>(|comment| comment.author_username.clone()));

    // Get the account metadata for the post authors
    let account_metadata_for_posts = AccountMetaData::get_account_metadata_batch(posts.map<Post, String>(|post| post.author_username.clone()));

    // Get the likes for the comments
    let comment_likes = comments.map<Comment, vector<Like>>(|comment| comment.likes.clone());
    ...
    return (account_metadata, comments, account_metadata_for_comments, posts, account_metadata_for_posts, viewer_comment_likes, viewer_post_likes);
}
    /*
...
public fun get_account_followers(
    ...
    // Get the followers list for the username
    let followers_list = Followers::get_followers_list(&username);

    // Get the start and end indices for the page
    let start_index = page_size * page_index;
    let end_index = min(start_index + page_size, followers_list.len());

    // Get the accounts that follow the username for the page
    let follower_accounts = followers_list.slice(start_index, end_index);

    // Get the account metadata for the username and the follower accounts
    let account_metadata = AccountMetaData::get_account_metadata(&username);
    let follower_account_metadata = AccountMetaData::get_account_metadata_batch(follower_accounts.clone());

    return (account_metadata, follower_account_metadata);
}
...
public fun get_account_followings(
    ...
    // Get the account metadata for the username and the followed accounts
    let account_metadata = AccountMetaData::get_account_metadata(&username);
    let followed_account_metadata = AccountMetaData::get_account_metadata_batch(followed_accounts.clone());

    return (account_metadata, followed_account_metadata);
}
...
public fun get_post(
    ...
    page_size: u64,
    page_index: u64
): (AccountMetaData, Post, bool, vector<Comment>, vector<AccountMetaData>) acquires State, AccountMetaData, Publications {
    // Get the post
    let post = Publications::get_post(&username, post_id);

    // Get the account metadata for the post author
    let account_metadata = AccountMetaData::get_account_metadata(&post.author_username);

    // Get the likes for the post
    let likes = post.likes.clone();

    // Get the viewer's like for the post
    let viewer_likes = likes.has_liked(&viewer_username);
    ...
    // Get the comments for the page
    let comments_page = comments.slice(start_index, end_index);

    // Get the account metadata for the comment authors
    let comment_authors = comments_page.map<Comment, String>(|comment| comment.author_username.clone());
    let comment_account_metadata = AccountMetaData::get_account_metadata_batch(comment_authors);

    return (account_metadata, post, viewer_likes, comments_page, comment_account_metadata);
}
